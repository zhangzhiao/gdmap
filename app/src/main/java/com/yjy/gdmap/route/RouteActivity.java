...
import com.yjy.gdmap.overlay.WalkRouteOverlay;
import com.yjy.gdmap.util.AMapUtil;
...
public class RouteActivity extends BaseActivity implements AMap.OnMapClickListener,
	...
	protected void onCreate(Bundle bundle) {
		...
		mStartPoint = new LatLonPoint(Double.parseDouble(sUtils.getString("mylat")), Double.parseDouble(sUtils.getString("mylng")));//起点，116.335891,39.942295
		mEndPoint=  new LatLonPoint(Double.parseDouble(sUtils.getString("endlat")), Double.parseDouble(sUtils.getString("endlng")));//终点，116.481288,39.995576
		Log.e("TAG", "onCreate: "+mEndPoint.getLatitude() +"-=-" +mEndPoint.getLongitude() );
		Log.e("TAG", "onCreate: "+mStartPoint.getLatitude() +"-=-" +mStartPoint.getLongitude() );
	...
	public void searchRouteResult(int routeType, int mode) {
		...
		if (routeType == ROUTE_TYPE_BUS) {// 公交路径规划
			BusRouteQuery query = new BusRouteQuery(fromAndTo, mode,
					sUtils.getString("city"), 0);// 第一个参数表示路径规划的起点和终点，第二个参数表示公交查询模式，第三个参数表示公交查询城市区号，第四个参数表示是否计算夜班车，0表示不计算
