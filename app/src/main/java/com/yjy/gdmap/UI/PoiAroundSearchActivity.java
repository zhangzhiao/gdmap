...
public class PoiAroundSearchActivity extends BaseActivity implements OnClickListener,
	 ...
	 */
	private void init() {
		lp = new LatLonPoint(Double.parseDouble(sUtils.getString("mylat")), Double.parseDouble(sUtils.getString("mylng")));//
		if (mAMap == null) {
	...
	private void setup() {
		...
		mPoiDetail.setOnClickListener(v -> {
...
//				startActivity(intent);
	...
	protected void doSearchQuery() {
		currentPage = 0;
		query = new PoiSearch.Query(keyWord, "", sUtils.getString("city"));// 第一个参数表示搜索字符串，第二个参数表示poi搜索类型，第三个参数表示poi搜索区域（空字符串代表全国）
		...
		if (lp != null) {
			...
			poiSearch.setBound(new SearchBound(lp, 1005000, false));//
	...
	public void onPoiSearched(PoiResult result, int rcode) {
		if (rcode == AMapException.CODE_AMAP_SUCCESS) {
			...
			ToastUtil
					.show(this.getApplicationContext(), "设备当前 GPS 状态差,建议持设备到相对开阔的露天场所再次尝试");
			ToastUtil
	...
	private void setPoiItemDisplayContent(final PoiItem mCurrentPoi) {
		...
		mPoiName.setText(mCurrentPoi.getTitle()+"(点击此处进行导航)");
		mPoiAddress.setText(mCurrentPoi.getSnippet());
		mPoiName.setOnClickListener(v->{
			MainActivity.getIn().tarNavi(new LatLng(mCurrentPoi.getLatLonPoint().getLatitude(),mCurrentPoi.getLatLonPoint().getLongitude()));
			finish();
		});
